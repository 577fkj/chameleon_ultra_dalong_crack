...
def download_file(url, save_path, chunk_size=8192, timeout=(5, 60)):
    ...
    with requests.get(url, stream=True, timeout=timeout, headers=headers) as r:
        ...
        file_name_tmp = r.headers.get("Content-Disposition", "").split("filename=")
        if len(file_name_tmp) > 1:
            file_name = file_name_tmp[1].strip('"\' ')
...
def download():
    ...
    try:
        ...
        data = response.json()
        print(f"Current Version: {version} Build Number: {build_number}")
        print("  Response:", data)
